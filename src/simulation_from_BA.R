...
output_dir <- file.path("output", "simulations_BA")
...
run_sim <- function(params) {
  ...
  for(i in 1:n_sim) {
    ...
    m <- max(1, s * (p - 1) / 2 + rbinom(1, 2, 0.25))
    alpha <- 1 + runif(1, 0, 100) * (rbinom(1, 1, 0.5) == 1)
    
    graph_sim <- igraph::ba.game(p, power = 1, m = m, directed = FALSE)
